...
def main():
	...
	def ransac(features1_all, features2_all, match_vec, tau=100, n_max=1000, inlier_percent=0.9, img1=None, img2=None):
		...
		inliers1 = inliers1[:,[1,0]]
		inliers2 = inliers2[:,[1,0]]
		print(inliers1)
	...
	H, inliers = ransac(n_best_features1,n_best_features2,matches,100)#,img1=img1,img2=img2)
	...
	def stitchImg(img1, img2, H):
		'''warp img2 to img1 with homograph H'''
		H_inv = np.linalg.inv(H)
		t = [600,600]
		Ht = np.array([[1,0,t[0]],[0,1,t[1]],[0,0,1]]) # translate

		im1 = cv2.warpPerspective(img1, Ht.dot(np.identity(3)), (img1.shape[1]*4, img1.shape[0]*4))
		im2 = cv2.warpPerspective(img2, Ht.dot(H_inv), (img2.shape[1]*4,img2.shape[0]*4))

		overlap_mask = np.any(im1 > 0, axis=2) & np.any(im2 > 0, axis=2)
		img_combo = (im1 * ~np.atleast_3d(overlap_mask)).astype(float) + im2.astype(float)

		cv2.imshow("mask1", cv2.resize(img_combo,(600,600)))
		cv2.waitKey()
		return img_combo

	def stitch(img1, img2, H):
		stitch = stitchImg(img1, img2, H)
		cv2.imwrite("stitch.png", stitch)
