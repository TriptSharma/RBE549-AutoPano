...
Parser.add_argument('--DataPath', default="../Data/P1TestSet/Phase1/TestSet1", help='Path to the dataset folder to stitch')
# TODO: tune these thresholds
Parser.add_argument('--MatchRatioThreshold', default=.75)
Parser.add_argument('--TauThreshold', default=100)
...
def ransac(corners1, corners2, matches_ij, img1=None, img2=None):
	...
	if img1 is not None:
		...
		cv2.imwrite("Results/ransac"+DataPath.split('/')[-1]+str(i)+str(j)+".png", out)
...
cv2.imwrite(DataPath.split('/')[-1]+".png", out[200:2100, :])

def viz(imgs, corners, best_corners, corner_descriptors):
	for i in range(len(imgs)):
		#save corner harris
		x = imgs[i].copy()
		x[corners[i] > 0.005 * corners[i].max()] = [0, 0, 255]
		f = DataPath.split('/')[-1]+str(i)+'.png'
		cv2.imwrite('Results/harris_'+f,x)
		#save anms
		x = imgs[i].copy()
		for pt in best_corners[i]:
			cv2.drawMarker(x, (pt[0], pt[1]), [0, 255, 0], cv2.MARKER_CROSS, 10, 1)
		cv2.imwrite('Results/anms_'+f,x)
		#save feature desc
		fig, axs = plt.subplots(5,5, figsize=(10,10))
		# print(corner_descriptors)
		# corner_descriptors_viz = corner_descriptors
		for j in range(5):
			for k in range(5):
				axs[j,k].imshow(corner_descriptors[0][j+k].reshape((8,8)))
		plt.savefig('Results/descriptors_'+f)
		print(str(i) + 'saved')

	print('All visualizations saved successfully')
viz(imgs,corners,best_corners,corners_descriptors)
